...
public class DatabaseConnection {
    ...
    private static ArrayList<RawMeasurement> getMeasurements(String address) {
        ...
        if (Files.exists(Paths.get(cacheFile))) {
            try {
                DataInputStream reader = new DataInputStream(new BufferedInputStream(new FileInputStream(cacheFile)));
        ...
        try {
            ...
            DataOutputStream cacheFileStream = new DataOutputStream(new BufferedOutputStream(new FileOutputStream(cacheFile)));
            ArrayList<RawMeasurement> measurements = new ArrayList<>();
            while (true) {
                RawMeasurement measurement = buildMeasurement(reader);
                if (measurement == null)
                    break;
                saveMeasurement(measurement, cacheFileStream);
                measurements.add(measurement);
            }
     ...
     */
    private static BufferedInputStream buildStream(String address) {
        try {
    ...
    private static RawMeasurement buildMeasurement(DataInputStream stream) {
        try {
            ...
            measurement.setDateStamp(LocalDateTime.ofEpochSecond(ts, 0, ZoneOffset.systemDefault().getRules().getOffset(Instant.ofEpochSecond(ts))));
    ...
    /**
     * Serializes and saves a measurement to a stream
     *
     * @param measurement the measurement to save
     * @param stream      the stream to save to
     */
    static private void saveMeasurement(RawMeasurement measurement, DataOutputStream stream) {

        try {
            stream.writeShort(measurement.getBarometer());
            stream.writeLong(measurement.getDateStamp().toEpochSecond(zone.getRules().getOffset(measurement.getDateStamp())));
            stream.writeShort(measurement.getInsideTemp());
            stream.writeShort(measurement.getInsideHum());
            stream.writeShort(measurement.getOutsideTemp());
            stream.writeShort(measurement.getOutsideHum());
            stream.writeShort(measurement.getWindSpeed());
            stream.writeShort(measurement.getAvgWindSpeed());
            stream.writeShort(measurement.getWindDir());
            stream.writeShort(measurement.getRainRate());
            stream.writeShort(measurement.getUVLevel());
            stream.writeShort(measurement.getSolarRad());
            stream.writeShort(measurement.getXmitBatt());
            stream.writeShort(measurement.getBattLevel());
            stream.writeShort(measurement.getSunrise());
            stream.writeShort(measurement.getSunset());
            stream.writeShort(Integer.parseInt(measurement.getStationId()));
        } catch (IOException e) {
            e.printStackTrace();
        }
    }
